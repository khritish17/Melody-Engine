...
bass_list = ft.ListView(spacing=10, auto_scroll=True, expand=True)

def refresh_bass_list():
    bass_list.controls.clear()

    for instrument in state.bass_instruments:
        bass_list.controls.append(
...
def bass_instruments_ui():
    return ft.Container(
        ...
        content= ft.Column(
            controls=[
                heading(),
                ft.Container(height=125, content=bass_list,),
...
def delete_clicked(e, config):
    state.bass_instruments.remove(config)
    refresh_bass_list()
    pass
...
def music_icon():
    return ft.Container(
        ...
        width=40,
        bgcolor="#E1FCEE",
        content=ft.Icon(icon=ft.Icons.PIANO, size=30, color="#32B97F"),
        border_radius=10,
        border=ft.Border.all(
            width=0.5,
            color="#6DB992"
...
def add_instument_clicked():
    state.bass_instruments.append(InstrumentConfig())
    refresh_bass_list()
...
def add_instrumen_button():
    return ft.OutlinedButton(
        ...
        #width=100,
        controls=[
            ft.Icon(ft.Icons.ADD, color="#32B97F"),
            ft.Text("Add Instrument", color="#32B97F"),
            ],
        ),
        style=ft.ButtonStyle(
            bgcolor="#E1FCEE",
...
def confirm_clicked():
    state.confirmed_bass_instruments = []
    for config in state.bass_instruments:
        ...
        state.confirmed_bass_instruments.append([instrument_program_no, velocity, delay])
    print(state.confirmed_bass_instruments)

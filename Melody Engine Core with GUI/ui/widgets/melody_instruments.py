...
melody_list = ft.ListView(spacing=10, auto_scroll=True, expand=True)

def refresh_melody_list():
    melody_list.controls.clear()

    for instrument in state.melody_instruments:
        melody_list.controls.append(
...
def melody_instruments_ui():
    return ft.Container(
        ...
        content= ft.Column(
            controls=[
                heading(),
                ft.Container(height=125, content=melody_list,),
...
def delete_clicked(e, config):
    state.melody_instruments.remove(config)
    refresh_melody_list()
    pass
...
def delete_button(config):
    return ft.OutlinedButton(
        ...
        style=ft.ButtonStyle(
            bgcolor=theme.background_color,
            side=ft.BorderSide(width=0.5,color="#FEB2D5"),
...
def instrument_dropdown(instrument_config):
    def instrument_changed(e):
        print("Melody Instrument Changed")
...
def music_icon():
    return ft.Container(
        ...
        width=40,
        bgcolor="#FEF1F7",
        content=ft.Icon(icon=ft.Icons.PIANO, size=30, color="#ED41A1"),
        border_radius=10,
        border=ft.Border.all(
            width=0.5,
            color="#FEB2D5"
...
def add_instument_clicked():
    state.melody_instruments.append(InstrumentConfig())
    refresh_melody_list()
...
def add_instrumen_button():
    return ft.OutlinedButton(
        ...
        #width=100,
        controls=[
            ft.Icon(ft.Icons.ADD, color="#ED41A1"),
            ft.Text("Add Instrument", color="#ED41A1"),
            ],
        ),
        style=ft.ButtonStyle(
            bgcolor="#FEF1F7",
...
def confirm_clicked():
    state.confirmed_melody_instruments = []
    for config in state.melody_instruments:
        ...
        state.confirmed_melody_instruments.append([instrument_program_no, velocity, delay])
    print(state.confirmed_melody_instruments)
